...
const StockFinder = class {
    constructor(hostRef) {
        ...
        this.rsOnSelectSymbol = index.createEvent(this, "rsOnSelectSymbol", 7);
    }
    render() {
        return [
            index.h("form", { id: "stock-form", onSubmit: this.findCorp }, index.h("label", null, "Nome da Empresa:"), index.h("input", { id: "stock-symbol", onInput: (e) => this.onInput(e) }), index.h("button", { type: "submit", disabled: this.input.length < 1 || this.loading }, "Pesquisar")),
            index.h("div", { id: "stock-list" }, !this.loading ?
                index.h("ul", null, this.searchResult.map(item => (index.h("li", { onClick: () => this.onSelectSymbol(item.symbol) }, index.h("b", { class: "symbol" }, item.symbol, ":"), index.h("span", null, item.name)))))
                : index.h("rs-spinner", null))
        ];
    }
};
...
const StockPrice = class {
    ...
    __stencil_render() {
        let content = this.loading ? index.h("rs-spinner", null) : index.h("p", { style: { color: 'red' } }, "Insira um simbolo valido!");
        if (this.price && !this.loading) {
            content = index.h("p", null, "Pre\u00E7o: ", `$${this.price}`);
        }
        if (this.error && !!this.loading) {
            content = index.h("p", { style: { color: 'red' } }, this.error);
        }
        return [
            index.h("form", { id: "stock-form", onSubmit: this.onSubmit }, index.h("label", null, "Simbolo:"), index.h("input", { id: "stock-symbol", value: this.symbol, onInput: (event) => this.onInput(event, 'symbol') }), index.h("button", { type: "submit", disabled: !this.validation['symbol'] || this.loading }, "fetch")),
            index.h("div", { class: "content-container" }, index.h("h3", { style: { minHeight: '17.6px' } }, this.symbol), content)
        ];
    }
...
exports.rs_spinner = Spinner;
exports.rs_stock_finder = StockFinder;
exports.rs_stock_price = StockPrice;
